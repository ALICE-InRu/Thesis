...
\begin{eqnarray} 
  \zeta_{\min}^{\pi} &:=& \Exp[\pi]{\given{\min(\rho)}{
      \forall C_{\max}^{\pi_\star(\vchi^{j})} \neq 
      C_{\max}^{\pi_\star(\vchi^{j^*})} \wedge j^* = \argmax_j\{\pi(\vphi^j)\}
      }} \\
  \zeta_{\max}^{\pi} &:=& \Exp[\pi]{\given{\max(\rho)}{
      \forall C_{\max}^{\pi_\star(\vchi^j)} \neq 
      C_{\max}^{\pi_\star(\vchi^{j^*})} \wedge j^* = \argmax_j\{\pi(\vphi^j)\} 
      }} \\
  \zeta_{\mu}^{\pi} &:=& \Exp[\pi]{\given{\rho}{
...
\Cref{fig:diff:case:SDR} depicts $\{\zeta_{\min}^{\pi},\zeta_{\max}^{\pi}\}$
for expert policy $\pi_\star$ and SDRs.
...
\begin{figure}[p]\centering
    ...
    illustrated jointly for \Problem{\text{train}} in \cref{fig:diff:case:OPT}}
...
$\zeta_{\mu}^{\text{SPT}}$ for \jrndJ{6}{5} or \jrnd{10}{10}. 
...
\jrnd{10}{10}  would be starting with $\xi^\star_{\text{SPT}}(k)$ and then 
...
$\zeta_{\mu}^{\text{SPT}}(40)$ has already surpassed 
$\zeta_{\mu}^{\text{MWR}}(K)$ and there are 60 operations left to dispatch. So 
a switch for BDR at $k=40$ never had a chance of improvement. 
However, at $k\leq15$ then $\zeta_{\mu}^{\text{SPT}}(k) < 
\zeta_{\mu}^{\text{MWR}}(k)$, which were appropriate turning points for BDR 
