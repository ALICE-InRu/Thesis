...
\end{equation}
As a result $\hat{\xi}_{\pi}$ is based on $\{\Phi^\pi,\mathcal{Y}^\pi\}$ 
meta-data from \cref{eq:trdat:metadata}.
...
steps $k=\{10,15,40\}$. Main statistics are given in \cref{tbl:BDR:stats}.
...
Changing to MWR at $k=10$ or $k=15$ is not statically significant from 
...
But this does tell us that $\hat{\xi}_\pi$ is a more reliable indicator than 
$\xi_\pi$ when it comes to choosing appropriate model parameters. 
Alas, $\hat{\xi}_\pi$ requires collecting the meta-data 
$\{\Phi^\pi,\mathcal{Y}^\pi\}$ from \cref{eq:trdat:metadata} for its policy 
$\pi$, whereas $\xi_\pi$ reuses $\{\Phi^{\pi_\star},\mathcal{Y}^{\pi_\star}\}$ 
