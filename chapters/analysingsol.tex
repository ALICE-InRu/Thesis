...
\begin{subequations}\label{eq:bwc:opt}
  \begin{eqnarray} 
  \zeta_{\min}^{\star} &:=& \Exp[\pi_\star]{\given{\min(\rho)}{
      \forall C_{\max}^{\vchi^j} \gneq C_{\max}^{\pi_\star} }} \\
  \zeta_{\max}^{\star} &:=& \Exp[\pi_\star]{\given{\max(\rho)}{
      \forall C_{\max}^{\vchi^j} \gneq C_{\max}^{\pi_\star} }}
  \end{eqnarray}
\end{subequations}
...
\begin{figure}\centering
  ...
  making
    \emph{one} suboptimal dispatch, depicted as lower and upper bound, 
...
optimum, $\xi^\star_{\text{RND}}$ (cf. \cref{fig:diff:opt:rnd}).
...
\begin{figure}\centering
  ...
  \caption[Probability of SDR yielding optimal move]{
    ...
    Both optimal (solid: $\xi^\star_{\langle \text{SDR}\rangle}$) and 
    SDR-based (dashed: $\xi_{\langle \text{SDR}\rangle}$) 
...
\end{equation}
\Cref{fig:diff:extr:SDR} shows the difference between $\xi^\star_{\langle 
\text{SDR} \rangle}$ and $\xi_{\langle \text{SDR} \rangle}$.
Similarly for \cref{eq:bwc:opt}, 
\begin{subequations}\label{eq:bwc:track}
\begin{eqnarray} 
  \zeta_{\min}^{\pi} := \Exp[\pi_\star]{\given{\min(\rho)}{
      \forall C_{\max}^{\pi_\star(\vchi^{j})} \neq 
      C_{\max}^{\pi_\star(\vchi^{j^*})} \wedge j^* = \argmax_j\{\pi(\vphi^j)\}
      }} \\
  \zeta_{\max}^{\pi} := \Exp[\pi_\star]{\given{\max(\rho)}{
      \forall C_{\max}^{\pi_\star(\vchi^j)} \neq 
      C_{\max}^{\pi_\star(\vchi^{j^*})} \wedge j^* = \argmax_j\{\pi(\vphi^j)\} 
      }}
\end{eqnarray}
\end{subequations}
Note, $\zeta^{\star}=\zeta^{\pi_{\star}}$.
As a result $\xi_{\pi}$ and $\{\zeta_{\min}^{\pi},\zeta_{\max}^{\pi}\}$ 
are based on $\{\Phi^\pi,\mathcal{Y}^\pi\}$ meta-data from 
\cref{eq:trdat:metadata}.
