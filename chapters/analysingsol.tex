...
used for testing is quite limited, and we saw from \cref{sec:jsp:example} that 
there are many jobs that have non-conflicting machines, thereby making 
subsequent dispatches equivalent to the previous one, i.e., $\vchi^k \approx 
\vchi^{(k\pm1)}$. Therefore it's reasonable to inspect the stepwise statistical 
testing based on sliding window of the preceding and subsequent step, i.e., 
test at time $k$ is based on: 
\begin{equation}
...
\begin{table}\centering
    ...
    \subcaptionbox{Used in \cref{fig:jrnd:feat:corr}}{
        \begin{tabular}{llrr}
            \toprule
            Track & Difficulty & N & $\#$Significant \\ 
            \midrule
            SPT & Easy &  37 &  78 \\ 
            SPT & Hard & 203 & 101 \\ 
            LPT & Easy & 103 &  87 \\ 
...
Furthermore, \cref{fig:jrnd:feat:corr} gives when easy or hard features 
