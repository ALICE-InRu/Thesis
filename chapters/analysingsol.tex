...
\cref{tbl:data}.
Furthermore, we know that our SDR heuristics aren't  perfect, and they're bound 
...
\end{equation}
As a result $\hat{\xi}_{\pi}$ is based on $\{\Phi^\pi,\mathcal{Y}\}$ meta-data 
from \cref{eq:trdat:metadata}.
\Cref{fig:diff:extr:SDR} shows the difference between $\xi_{\langle \text{SDR} 
    \rangle}$ and $\hat{\xi}_{\langle \text{SDR} \rangle}$.
...
${\xi}_{\text{SPT}}$ always outperforms ${\xi}_{\text{LPT}}$ in choosing a 
...
\begin{figure}\centering
   \captionsetup{list=no}
    ...
    \caption[Probability of SDR yielding optimal move]{
        sProbability of SDR yielding optimal move. Both optimal 
    (solid: $\xi_{\langle \text{SDR}\rangle}$) and SDR-based 
    (dashed: $\hat{\xi}_{\langle \text{SDR}\rangle}$) trajectories are 
    inspected.}\label{fig:diff:extr:SDR}
\end{figure}
\begin{figure}[t]\centering
    ...
    \subcaptionbox{$10\times10$\label{fig:diff:extr:SDR:10x10}}{
      ...
      \node (zoom) at (0.27,.45)
      {\includegraphics[width=0.24\textwidth]{figures/j.rnd/{stepwise.10x10.SDR.zoom}.pdf}};
      \end{scope}
      \end{tikzpicture}}
...
\jrnd{10}{10}  would be starting with ${\xi}_{\text{SPT}}$ and then switching 
...
Revisiting \cref{fig:diff:extr:SDR:10x10} for \jrnd{10}{10}, we can see that 
even though $\hat{\xi}_{\text{SPT}}$ is generally more likely to find optimal 
dispatches in the initial steps, then shortly after $k=15$, 
$\hat{\xi}_{\text{MWR}}$ becomes a contender again. 
This could explain why our BDR switch at $k=40$ in 
\cref{fig:diff:boxplot:BDR} was unsuccessful. 
However, changing to MWR at $k=10$ or $k=15$ is not statically significant from 
MWR (boost in mean $\rho$ is at most 1.25\%). 
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/{j.rnd}/{boxplotRho.BDR.10x10}.pdf}
  \caption{Box plot of \jrnd{10}{10} \namerho, for BDR where SPT is applied 
  for the first 10\%, 15\% or 40\% of the dispatches, followed by MWR.}
...
$\Phi^{\langle\text{SDR}\rangle}$ (conditioned on the followed trajectory) for 
\jrnd{6}{5} are given in \cref{tbl:jrnd:feat:cnt}.
...
\begin{figure}[t]
    \includegraphics[width=\textwidth]{{j.rnd/phi.ks.SDR.6x5}.pdf}
...
\begin{figure}
    \includegraphics[width=\textwidth]{{j.rnd/phi.corr.SDR.6x5}.pdf}
    \includegraphics[width=\textwidth]{{j.rnd/phi.corr.ALL.6x5}.pdf}
