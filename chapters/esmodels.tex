...
used for \cref{eq:CDR:feat} on the corresponding training and test data. 
...
deviation, minimum and maximum values. Furthermore, 
\cref{fig:cma:boxplot:orlib} depicts the box-plot when applying the models (for 
all training data) on the benchmark problem instances from the OR-Library.
...
\begin{figure}[t]\centering
    \includegraphics[width=\columnwidth]{{CMAES.generation.fitness}.pdf}
    \caption{Fitness for optimising w.r.t. \cref{eq:cma:objfun}, per generation 
    of the CMA-ES optimisation.}\label{fig:cma:fit}
\end{figure}
