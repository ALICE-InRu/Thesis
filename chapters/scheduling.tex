...
comprehend it via graph theory.
Let's encode the operations as vertices, and horizontally aligning them w.r.t. 
each job $J_j$, and connect vertices with directed edges according the 
\cref{eq:permutation}. 
By introducing dummy vertices before and after, then the goal is to visit 
each vertex exactly once, or \emph{Hamiltonian} path: starting at the `source' 
(i.e. empty schedule), and finishing at the sink (i.e. complete schedule).
...
and the directed edges indicate the current partial Hamiltonian path.
...
temporal partial schedule of \cref{eq:vchi:example} as a Gantt-chart
\begin{enumerate*}
...
\end{enumerate*}
Note, the sequence from \cref{fig:example:graph:midway} gives the schedule in
\cref{fig:example:midway}.
...
to pilot method \cite{Geirsson2012}}
