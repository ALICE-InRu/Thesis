...
heuristics, where one starts with an empty schedule and adds sequentially on 
one operation (or task) at a time. 
Namely, at each time step $k$, an operation is dispatched which has the highest 
priority of the job-list, \mbox{$\mathcal{L}^{(k)}\subset\mathcal{J}$}, i.e., 
...
will refer to the sequential ordering of the dispatches of tasks to machines, 
namely, 
\begin{equation}
  \vchi = \big\{\chi_k\big\}_{k=1}^\ell
        = \condset{(j,a)}{J_j\in\mathcal{L}^{(k)}}_{k=1}^\ell
\end{equation}
Note, only a sequence of $J_j$ is needed, since the corresponding $M_a$ can be 
obtained by reading $\vsigma$. 
The collective set of allocated tasks to machines, which is interpreted by its 
sequence, is referred to as a \emph{schedule}; and a \emph{scheduling policy} 
$\pi$ will pertain to the manner in which the sequence is manufactured: be it a 
...
How should the guests go about 
their tea-party, in order for Alice to be on-time?
...
basis.  \todoExtend{These features have not been published yet}
