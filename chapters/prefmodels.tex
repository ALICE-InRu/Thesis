...
\begin{enumerate}[after={{}}, leftmargin=*,
    ...
    \item \textbf{(equal)} \label{bias:equal} equal probability. This was the 
    sampling 
...
achiever better results than applying a local model for each time step. 
In fact, for \jrndn{10}{10} a stepwise model was the worst approach. 
...
\begin{enumerate*}
    \item \jrnd{10}{10} by 14\% using adjusted \ref{bias:prefsize}
    \item \jrndn{10}{10} by 1\% using adjusted \ref{bias:opt} (not     
    significant improvement)
    \item \frnd{10}{10} by 8\% using adjusted \ref{bias:prefsize}
\end{enumerate*}
