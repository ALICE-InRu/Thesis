...
\section{Stepwise sampling bias}
...
time step?
Following strategies for stepwise bias will now be proportional to
\begin{enumerate}[after={{}}, leftmargin=*,
    label={\textbf{Bias.\arabic*}}, ref={{Bias.\arabic*}}]
    \item \label{bias:equal} equal probability (equal). This was the sampling 
    strategy for \PsiSet{p} in \cref{InRu14,InRu15a} and will serve as a 
    baseline.  
    \item \label{bias:opt} inverse optimality for random dispatches, i.e., 
    $1-\xi_{\RND}^{\star}$ (opt).
    Motivation being to to give samples from dispatches that are less 
    likely to be optimal than simply at random (cf. \cref{fig:diff:opt:rnd}). 
    \item \label{bias:bcs} best case scenario for mean $\rho$, i.e., 
    $\zeta^{\star}_{\min}$ (bcs).
    \item \label{bias:wcs} worst case scenario for mean $\rho$, i.e., 
    $\zeta^{\star}_{\max}$ (wcs).
    \item \label{bias:featset} inversely proportional to $\abs{\Phi^{\OPT}}$ 
    (featsize), defined as
    $$\max_{k'}\{\abs{\Phi^{\OPT}(k')}\}-\abs{\Phi^{\OPT}(k)}+\min_{k}\{\abs{\Phi^{\OPT}(k')}\}$$
    \item \label{bias:prefset} inversely proportional to $\abs{\Psi^{\OPT}_p}$ 
    (prefsize), defined as  
...
Note, \ref{bias:bcs} and \ref{bias:wcs} are more focused on 
...
Whereas, \ref{bias:featset} and \ref{bias:prefset} are trying to give more 
equal emphasis on stepwise features, as they substantially decrease over time 
(cf. \cref{fig:size:Phi:K,fig:size:Psi.p:K}). 
...
\cref{tbl:bias:boxplot}. In addition to the stepwise bias strategies (both 
adjusted and not) a stepwise model (one for 
each step $k$) is given for reference.

There we can see that for \jrnd{10}{10} the best model is a stepwise one, as 
expected 
