...
all other \Problem[6\times5]{\text{train}} in \cref{tbl:data}).
...
outperform the original $\Psi_b$ set-up from \cref{InRu11a}. 
...
The different trajectories for all \Problem[6\times5]{\text{train}} and 
\jrnd{10}{10} are now ordered w.r.t. their mean \namerho, and is 
given in \cref{tbl:param:tracks}. Models that are statistically different are 
denoted by `$\succ$', otherwise considered equivalent. Note, box-plots for 
\jrnd{6}{5} and \frnd{6}{5} were given in \cref{fig:boxplot:prefset}.
