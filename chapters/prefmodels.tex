...
\label{sec:param:tracks:passive}

Let's limit the to models corresponding to $\Phi_p$ in 
\cref{fig:boxplot:prefset}. 
...
Looking back at \cref{fig:size:Phi:K}, then even though \PhiSet{LPT} not the 
trajectory with the least features, the amount of equivalent features w.r.t. 
$C_{\max}$ are far too many to make a meaningful preference set out of it. 
It's only for \jrndn{6}{5} that there is another trajectory with fewer 
preferences, namely \PsiSet{LWR}{p} (cf. \cref{fig:size:Psi:K}), and in that 
case LWR is worst model.
...
preference set then becomes too varied for an satisfactory result. 
...
\begin{figure}[t]
  ...
  \caption[Size of preference set, $\abs{\Psi_p}$]{Size of 
    \Problem[6\times5]{\text{train}} preference set, $l=\abs{\Psi_p}$, for 
...
%     j.rnd     10x10 Track Track Track  SAME Track    N/A     N/A

When comparing $\Psi{\langle\text{DR}\rangle}$ with its corresponding DR, then 
usually the preference model outperformed the DR it was trying to mimic. 
The exceptions being 
\begin{enumerate*}
  \item MWR for \jrndJ{6}{5} and \jrndM{6}{5}. Note \jrnd{10}{10} was 
...
The rational for using the $\Phi^{\langle \text{CMA-ES} \rangle}$ strategies 
...
However, this strategy is not outperformed the original DR used in guiding the 
training data collection. 



\subsection*{Summary and conclusion}
As the experimental results showed, the ranking of optimal\footnote{Here the
  tasks labelled `optimal' do not necessarily yield the optimum makespan except 
  in the case of following expert policy $\pi_\star$), instead these are the 
  optimal dispatches for the given partial schedule.} 
...
the learning algorithm could learn. 
