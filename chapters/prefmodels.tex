...
a training set of $N_{\text{train}}=500$ problem instances of \jrnd{6}{5} and 
...
First let's restrict  the models to \Problem{6\times5}{\text{train}}. 
...
\Problem[6\times5]{\text{train}}. Only exception begin that ranking schemes 
showed difference in performance when using \PhiSet{OPT}, where 
$\Psi_{p}^{\text{OPT}}$ come on top. Strengthening our previous choice of 
$\Psi_{p}$ as standard ranking scheme.
...
However, this strategy is not outperforming the original DR used in guiding the 
training data collection. 

\subsection*{Summary and conclusion}
As the experimental results showed, the ranking of optimal
