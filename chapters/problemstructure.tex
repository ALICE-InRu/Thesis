...
\namerho. \todoFind{K-S tests are not used in this chapter, move until later}
...
\todoWrite{Anoter way could be inspecting the number of Simplex iterations 
needed for an optimal schedule. However, preliminary experiments showed that an 
increased amount of Simplex solutions didn't transcend to high $\rho$, if 
anything it means there are many optimal or near-optimal solutions available, 
hence there is a slow process of eliminating branches of the tree, before 
reaching to a final incumbent solution. }
