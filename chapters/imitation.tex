...
\PhiSet{\IL{i}}, hence the aggregated feature set \PhiSet{\DA{i}} is only 
...
and the corresponding \ref{bias:equal} from \cref{fig:boxplot:active} are shown 
together. 
...
\begin{enumerate*}  
	...
	$T=7$
	\item on the other hand when using adjusted \ref{bias:dbl2nd}, only one 
...
\ref{bias:equal} there was still $\Delta\rho\approx5.13\%$ difference, and for 
$i=2$ for adjusted \ref{bias:dbl2nd} that difference was almost halved, or 
$\Delta\rho\approx2.8\%$ compared to optimising \cref{eq:cma:makespan}. 
...
(LOLS) by \cite{ChangKADL15}, it is possible to use imitation learning (similar 
to DAgger framework) when the reference policy is poor (i.e. $\pi_\star$ in 
