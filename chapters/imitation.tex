...
even though $\abs{\Phi^{\DA{i}}}\gg\abs{\Phi^{\OPT}_{\text{EXT}}}$ for \jrnd{10}{10} after $i>2$. This goes to show that `too much' data is no longer a bad influence. 
...
For \ref{DA:FIXSUP} there was no further improvement over all $T=7$ iterations using \jrnd{6}{5} (therefore not considered for \jrnd{10}{10}).
For \ref{DA:SUP} then significant improvement was achieved at $i=5$ using \jrnd{6}{5}.
However, for \jrnd{10}{10} then no significant improvement was after $i>1$. 
On the other hand, \ref{DA:UNSUP} had some unexpected performance for \jrnd{6}{5} as it  deteriorated for $i\in\{2,3\}$, but at $i=4$ it got back on track before stagnating.
...
For other \Problem[6\times5]{\train} problem spaces\footnote{Consult \shiny: Preference models $>$ Imitation Learning} then there was not much difference in \ref{DA:SUP} and \ref{DA:UNSUP}, although the latter was slightly better for \frnd{6}{5}.
Note, for \fsp\ problem spaces then DAgger was not fruitful, as either the iterations were statistically insignificant from the model obtained from the expert policy, or performance slightly downgraded with each iteration. Although, it's noted that those experiments were done with reusing the same $N_{\train}$ problem spaces over and over again (i.e. not applying \ref{Boost:EXT}).
...
Box-plot for \namerho, is depicted in \cref{fig:all:boxplot}, and main statistics are given in \cref{tbl:IL:stats}. Both adjusted \ref{bias:dbl2nd} and the corresponding \ref{bias:equal} from \cref{fig:boxplot:active} are shown together. 
...
\begin{enumerate*}  
	...
	$i=2$ and after that $\Delta\rho\approx-1.31\%$ until the final iteration $T=7$
...
\begin{figure}[t]
	...
	\caption{Box plot for \jrnd{10}{10} \namerho, using either expert policy, 
		DAgger or following perturbed leader strategies. }\label{fig:all:boxplot} 
\end{figure}
...
\begin{enumerate*}
	...
	\item imitation learning, trained on $\Phi^{\DA{i}}_{\text{EXT}}$ ($i\leq7$ for \ref{bias:equal} and $i\leq2$ for adjusted \ref{bias:dbl2nd})
\end{enumerate*}\linebreak
As a reference, $\minCmax$ model from optimising \cref{eq:cma:makespan} and MWR are shown on the far right of \cref{fig:all:boxplot}.
...
When compared to $\minCmax$, then neither extended imitation learning approaches outperformed the direct optimisation. After $T=7$ iterations for \ref{bias:equal} there was still $\Delta\rho\approx5.13\%$ difference, and for $i=2$ for adjusted \ref{bias:dbl2nd} that difference was almost halved, or $\Delta\rho\approx2.8\%$ compared to optimising \cref{eq:cma:makespan}. 
