...
Thereby justifying only considering the `easy'\footnote{Easy in terms of 
computational effort.} \JSP\ before investing valuable time for higher 
...
\todoWrite{Instead of doing fixed roll-outs based on SDRs, as \phiSDRRelated, 
    then it could be worth investigating a single roll-out of learned policy, 
    $\hat{\pi}$. Generally, the learned policy surpasses SPT (i.e. $-\phiproc$) 
    w.r.t. stepwise optimality, i.e., 
    $\xi^{\star}_{\hat{\pi}}\geq\xi^{\star}_{-\phiproc}$. Moreover, we saw how 
    pursuing $-\phiSPT$, which is basically repeatedly applying $-\phiproc$ for 
    a 
    $(K-k)$-lookahead, significantly increased performance in 
    \cref{fig:rollout1:boxplot}.
}
