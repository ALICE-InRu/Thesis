...
\qauthor{The Mock Turtle} 
\end{savequote}

\chapter{Ordinal regression}\label{ch:ordinal} \todo[color=green!40]{\cref{ch:ordinal} Unfinished}
...
\begin{equation}
S = \left\{\left(\vphi(\vec{x}_k^{(1)}), \vphi(\vec{x}_k^{(2)})\right),t_k=\text{sign}(y_k^{(1)} - y_k^{(2)})\right\}_{k=1}^{l'} \subset \Phi \times Y \label{eq:Sfeat}.
\end{equation}
...
Logistic regression learns the optimal para\-meters $\vec{w}\in\mathbb{R}^d$ determined by solving the following task,
...
In the case that the training set $S$ defined by~\eqref{eq:Sfeat} is not linearly separable, a common way of coping with non-linearity is to apply the ``kernel-trick'' to transform $S$ onto a higher dimension. In which case, the dot product in \eqref{eq:linear} is replaced by a kernel function $\kappa$.

In terms of training data, the optimal $\vec{w}^*$ can be expressed as, \todo{Finna uppr. heimild}
\begin{equation}
\vec{w}^*=\sum_{k=1}^{\abs{S}} \alpha^* t_k \left( \vphi(\vec{x}_k^{(1)})-\vphi(\vec{x}_k^{(2)}) \right)
\end{equation}
and the function $h$ may be reconstructed as follows,
\begin{eqnarray}\label{eq:nonlinear}
h(\vec{x})=\inner{\vec{w}^*}{\vphi(\vec{x})} &=& 
\sum_{k=1}^{\abs{S}} \alpha^* t_k \left( \inner{\vphi(\vec{x}_k^{(1)})}{\vphi(\vec{x})}-\inner{\vphi(\vec{x}_k^{(2)})}{\vphi(\vec{x})} ) \right) \nonumber \\ 
&=&\sum_{k=1}^{\abs{S}} \alpha^* t_k \left( \kappa(\vec{x}_k^{(1)},\vec{x})-\kappa(\vec{x}_k^{(2)},\vec{x}) \right)
\end{eqnarray}
...
\begin{equation}
\max_{\alpha} \sum_{k=1}^{\abs{S}} \alpha_k -\frac{1}{2} \sum_{i=1}^{\abs{S}}\sum_{j=1}^{\abs{S}} \alpha_i\alpha_jt_it_j\left(K(\vec{x}_i^{(1)},\vec{x}_i^{(2)},\vec{x}_j^{(1)},\vec{x}_j^{(2)}) +\frac{1}{C}\delta_{ij}\right)
\end{equation} 
subject to $\sum_{k=1}^{\abs{S}} \alpha_kt_k=0$ and $\alpha_k\geq0$ for all $k\in\{1,\ldots,\abs{S}\}$, and where,
\begin{eqnarray*}
...
In the case of JSSP, scaling makes the features less sensitive to varying problem instances.
Moreover, for surrogate modelling, it is important to scale the features $\vphi$ as the evolutionary search zooms in on a particular region of the search space. 
...
\begin{comment}
\section{Ordinal regression for surrogate modelling}\label{ordinal:surrogates}
Then the surrogate considered may be defined by a linear function in the kernel-defined feature space:
\begin{equation}
  h(\vec{x}) = \sum_{i=1}^d w_i\phi_i(\vec{x}) = \inner{\vec{w}}{\vphi(\vec{x})}.
\end{equation}
where $\vec{w}=[w_1,...,w_d]\in\mathbb{R}^d$ has weight $w_i$ corresponding to feature $\phi_i$.
...
\begin{equation} 
  \min_{\vec{w}}\quad \tfrac{1}{2}\inner{\vec{w}}{\vec{w}} + \tfrac{C}{2}\sum_{k=1}^{l'}\xi_k^2
\end{equation}
subject to $t_k\inner{\vec{w}}{(\vphi(\vec{x}_k^{(1)})-\vphi(\vec{x}_k^{(2)})}\ge 1 - \xi_k$ and $\xi_k \ge 0$, $k = 1,\ldots, l'$. The degree of constraint violation is given by the margin slack variable $\xi_k$ and when greater than $1$ the corresponding pair are incorrectly ranked. Note that
\begin{equation} 
  h(\vec{x}_i)-h(\vec{x}_j) = \inner{\vec{w}}{(\vphi(\vec{x}_i)-\vphi(\vec{x}_j))}  
\end{equation}
