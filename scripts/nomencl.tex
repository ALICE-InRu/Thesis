...
performance space, i.e., $y=\Upsilon(a,\vphi(\vec{x}))\in\mathcal{Y}$ where 
$\Upsilon:\mathcal{A}\times\mathcal{F}\mapsto\mathcal{Y}$}
...
\nomenclature[x01]{\PsiSet{b}}{preference set added w.r.t. basic ranking}
\nomenclature[x02]{\PsiSet{f}}{preference set added w.r.t. full subsequent 
ranking}
\nomenclature[x03]{\PsiSet{p}}{preference set added w.r.t. partial subsequent 
ranking}
\nomenclature[x04]{\PsiSet{a}}{union of all aforementioned rankings, i.e., 
$\Psi_{a}=\Psi_b\cup \Psi_f\cup \Psi_p$}
\nomenclature[x05]{\PhiSet{\SPT}}{training data guided by SPT trajectory}
\nomenclature[x06]{\PhiSet{\LPT}}{training data guided by LPT trajectory}
\nomenclature[x07]{\PhiSet{\LWR}}{training data guided by LWR trajectory}
\nomenclature[x08]{\PhiSet{\MWR}}{training data guided by MWR trajectory}
\nomenclature[x09]{\PhiSet{\OPT}}{training data guided by (random) optimum 
trajectory}
\nomenclature[x10]{\PhiSet{\RND}}{training data guided by a random trajectory}
\nomenclature[x11]{\PhiSet{\CMAES}}{training data guided by trajectory using by 
CMA-ES obtained weights, either \PhiSet{\minCmax} or \PhiSet{\minRho}}
\nomenclature[x12]{\PhiSet{\ALL}}{union of all aforementioned trajectories, 
i.e.,  $\PhiSet{\ALL}=\Phi^{\SDR}\cup \Phi^{\CMAES}$}
...
\nomenclature[y06]{OPT}{(known) optimum}
\nomenclature[y07]{BKS}{best known solution}
...
\nomenclature[z08]{LPT}{Largest Processing Time rule}
