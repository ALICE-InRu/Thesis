\newcommand{\Alice}{ALICE}
\hyphenation{\Alice}
\newcommand{\fullnameAlice}{Analysis \& Learning Iterative Consecutive 
Executions}
\newcommand{\shiny}{\href{http://tgax89.rhi.hi.is:3838/alice}{Shiny 
application}}

% colors styles for ColorBrewer Set1 - to match ggplot2
\definecolor{set1red}{RGB}{228, 26, 28} %1 #E41A1C
\definecolor{set1blue}{RGB}{55, 126, 184} %2 #377EB8
\definecolor{set1green}{RGB}{77, 175, 74} %3 #4DAF4A
\definecolor{set1purple}{RGB}{152, 78, 163} %4 #984EA3
\definecolor{set1orange}{RGB}{255, 127, 0} %5 #FF7F00
\definecolor{set1yellow}{RGB}{255, 255, 51} %6 #FFFF33
\definecolor{set1brown}{RGB}{166, 86, 40} %7 #A65628
\definecolor{set1pink}{RGB}{247, 129, 191} %8 #F781BF

%\renewcommand{\rho}{\varrho}
\renewcommand{\vec}[1]{\mathbf{#1}}
\newcommand{\vphi}{{\mathbf{\phi}}}
\newcommand{\vchi}{{\mathbf{\chi}}}
\newcommand{\vsigma}{{\mathbf{\sigma}}}
\newcommand{\mat}[1]{\mathbf{#1}}
\newcommand{\R}{{\mathbb R}}
\newcommand{\Exp}[2][ ]{{\mathbb E}_{#1}\Big\{#2\Big\}}
\newcommand{\Prob}[1]{{\mathcal P}\left(#1\right)}
\newcommand{\strng}[1]{{\mbox{\tt #1}}}
\newcommand{\inner}[2]{\big<{#1}\cdot{#2}\big>}
\newcommand{\abs}[1]{\lvert#1\rvert}
\newcommand{\norm}[1]{\lVert#1\rVert}
\newcommand{\argmax}{\mathop{\rm argmax}}
\newcommand{\argmin}{\mathop{\rm argmin}}
\newcommand{\condset}[2]{\left\{\given{#1}{#2}\right\}}
\newcommand{\given}[2]{#1\;:\;#2}
\newcommand{\PhiSet}[1]{$\Phi^{#1}$}
\newcommand{\PsiSet}[2][]{$\Psi^{#1}_{#2}$}

\newcommand{\train}{\text{train}}
\newcommand{\test}{\text{test}}
\newcommand{\MWR}{\text{MWR}}
\newcommand{\LWR}{\text{LWR}}
\newcommand{\SPT}{\text{SPT}}
\newcommand{\LPT}{\text{LPT}}
\newcommand{\RND}{\text{RND}}
\newcommand{\OPT}{\text{OPT}}
\newcommand{\ALL}{\text{ALL}}
\newcommand{\IL}[1]{\text{IL}#1}
\newcommand{\DA}[1]{\text{DA}#1}
\newcommand{\angles}[1]{\langle #1 \rangle}
\newcommand{\any}{\angles{\cdot}}
\newcommand{\CMAES}{\angles{\text{CMA-ES}}}
\newcommand{\SDR}{\angles{\text{SDR}}}

\newcommand{\limit}[3]{\lim_{#2}#1=#3}
\newcommand{\bigOh}[1]{\mathcal{O}\left(#1\right)}

% percentage relative deviation from optimality
\newcommand{\Namerho}{Deviation from optimality, $\rho$}
\newcommand{\namerho}{deviation from optimality, $\rho$}
\newcommand{\fullnamerho}{\namerho, defined by~\cref{eq:rho}}
\newcommand{\ProblemSpace}[2][ ]{$\mathcal{P}_{#2}^{#1}$}
\newcommand{\jrnd}[2]{\ProblemSpace[#1 \times #2]{j.rnd}}
\newcommand{\jrndJ}[2]{\ProblemSpace[#1 \times #2]{j.rnd,J_1}}
\newcommand{\jrndM}[2]{\ProblemSpace[#1 \times #2]{j.rnd,M_1}}
\newcommand{\jrndn}[2]{\ProblemSpace[#1 \times #2]{j.rndn}}
\newcommand{\frnd}[2]{\ProblemSpace[#1 \times #2]{f.rnd}}
\newcommand{\frndn}[2]{\ProblemSpace[#1 \times #2]{f.rndn}}
\newcommand{\fjc}[2]{\ProblemSpace[#1 \times #2]{f.jc}}
\newcommand{\fmc}[2]{\ProblemSpace[#1 \times #2]{f.mc}}
\newcommand{\fmxc}[2]{\ProblemSpace[#1 \times #2]{f.mxc}}

\newcommand{\minCmax}{\text{ES}.{C_{\max}}}
\newcommand{\minRho}{\text{ES}.{\rho}}

\newcommand{\dr}{dispatching rule}
\newcommand{\cdr}{composite priority \dr}
\newcommand{\sdr}{single priority \dr}
\newcommand{\Fsp}{Flow-shop}
\newcommand{\Jsp}{Job-shop}
\newcommand{\fsp}{flow-shop}
\newcommand{\jsp}{job-shop}
\newcommand{\FSP}{FSP}
\newcommand{\JSP}{JSP}
\newcommand{\Jrnd}{\JSP~random}
\newcommand{\JrndJ}{\JSP~random with job variation}
\newcommand{\JrndM}{\JSP~random with machine variation}
\newcommand{\Jrndn}{\JSP~random-narrow}
\newcommand{\Frnd}{\FSP~random}
\newcommand{\Frndn}{\FSP~random narrow}
\newcommand{\Fjc}{\FSP~job-correlated}
\newcommand{\Fmc}{\FSP~machine-correlated}
\newcommand{\Fmxc}{\FSP~mixed-correlated}

% job-related
\newcommand{\phiproc}{\phi_1}
\newcommand{\phistartTime}{\phi_2}
\newcommand{\phiendTime}{\phi_3}
\newcommand{\phiarrivalTime}{\phi_4}
\newcommand{\phiwait}{\phi_5}
\newcommand{\phijobTotProcTime}{\phi_6}
\newcommand{\phijobWrm}{\phi_7}
\newcommand{\phijobOps}{\phi_8}
\newcommand{\phiJobRelated}{$\{\phi_i\}_{i=1}^{8}$}
% mac-related
\newcommand{\phimacFree}{\phi_{9}}
\newcommand{\phimacTotProcTime}{\phi_{10}}
\newcommand{\phimacWrm}{\phi_{11}}
\newcommand{\phimacOps}{\phi_{12}}
\newcommand{\phireducedSlack}{\phi_{13}} 
\newcommand{\phimacSlack}{\phi_{14}}
\newcommand{\phiallSlack}{\phi_{15}}
\newcommand{\phiSlackRelated}{$\{\phi_i\}_{i=13}^{15}$}
\newcommand{\phimakespan}{\phi_{16}}
\newcommand{\phiMacRelated}{$\{\phi_i\}_{i=9}^{16}$}
% global
\newcommand{\phiSPT}{\phi_{17}}
\newcommand{\phiLPT}{\phi_{18}}
\newcommand{\phiLWR}{\phi_{19}}
\newcommand{\phiMWR}{\phi_{20}}
\newcommand{\phiRND}{\phi_{\RND}}
\newcommand{\phiRNDmean}{\phi_{21}}
\newcommand{\phiRNDstd}{\phi_{22}}
\newcommand{\phiRNDmin}{\phi_{23}}
\newcommand{\phiRNDmax}{\phi_{24}}
% summary
\newcommand{\NrFeatLocal}{16}
\newcommand{\NrFeatSDR}{20}
\newcommand{\NrFeatGlobal}{24}
\newcommand{\phiSDRRelated}{$\{\phi_i\}_{i=17}^{\NrFeatSDR}$}
\newcommand{\phiRNDRelated}{$\{\phi_i\}_{i=21}^{\NrFeatGlobal}$}
\newcommand{\phiGlobalRelated}{$\{\phi_i\}_{i=17}^{\NrFeatGlobal}$}
\newcommand{\phiLocalRelated}{$\{\phi_i\}_{i=1}^{\NrFeatLocal}$}

\hyphenpenalty=800
\hbadness=2500

\hyphenation{heur-ist-ics}
\hyphenation{algo-rithm}

\usepackage[inline]{enumitem}
\setlist[enumerate,1]{
    label=\textit{\roman*)},
    before=\unskip{: }, 
    itemjoin={{; }}, itemjoin*={{, and }},
    after={{. }}
}

\newcommand{\openoddpage}{
    \strictpagecheck
    \checkoddpage
    \ifoddpage 
        %do nothing
    \else 
        \blankpage
    \fi
}